// Exact-source build provenance for generated server and CLI artifacts.
//
// `write` runs at the end of the clean full build (`npm run build:server:clean`)
// and stamps packages/server/dist and packages/cli/dist with the Git commit the
// artifacts were built from. `invalidate` runs on the incremental
// `npm run build:server` path and deletes the receipts, because an incremental
// rebuild can leave outputs no single commit can vouch for. Desktop packaging
// refuses dist trees whose receipts are missing, dirty, or disagree with HEAD
// (see packages/desktop/scripts/build-provenance-gate.js), so a stale dist can
// never reach a packaged app silently.
//
// CommonJS on purpose: the Electron Builder hook scripts in
// packages/desktop/scripts are CommonJS and require this module directly so
// writer and verifier can never drift on the receipt format.
...
// packages/desktop/package.json before building. That file cannot change the
// server/CLI artifacts, so a version-only diff there is the one tracked change
// an exact-source build accepts.
...
function collectExactSourceState(workspaceRoot) {
  ...
  let dirtyPaths = runGit(workspaceRoot, ["status", "--porcelain"])
    .split("\n")
    .filter(Boolean)
...
function formatBuildReceipt(state) {
  return `${JSON.stringify({ commit: state.commit, dirty: state.dirtyPaths }, null, 2)}\n`;
}
    ...
    if (!fs.existsSync(path.dirname(target))) {
      throw new Error(
        `Cannot stamp @getpaseo/${packageName}: ${path.dirname(target)} does not exist. ` +
          "Receipts are written only after the full clean build produced dist.",
      );
    }
...
function assertExactSourceReceipts(workspaceRoot) {
  ...
  for (const packageName of RECEIPT_PACKAGES) {
    ...
    if (!fs.existsSync(target)) {
      throw new Error(
        `@getpaseo/${packageName} dist carries no ${RECEIPT_BASENAME}, so it was not produced ` +
          "by the clean full build. Run npm run build:server:clean at HEAD before packaging.",
      );
    }
