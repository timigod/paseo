...
workspace or worktree. All destructive aliases share the same authorization rules. Each operation
checks once after resolving its target and checks the captured caller again immediately before every
irreversible mutation that follows an awaited lookup, storage, cancellation, or teardown boundary. The
...
target is live. For restored or imported agents without `workspaceId`, and for partial callers whose
target has no workspace record, the same fence compares canonical cwd and Git worktree-root evidence
for live agents with every target checkout, including symlink aliases and nested or sibling paths.
Deprecated per-action caller fields remain bounded and parseable but are ignored.
...
its authority is scoped to one request and is revoked when either the request aborts or the response
closes. In-flight MCP handlers receive the request abort signal and also recheck the captured
authority before mutation. Agent-scoped MCP claims remain restricted agents. Neither form is serialized into provider
