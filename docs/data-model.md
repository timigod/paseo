...
| `lastStatus`         | `AgentStatus`                            | One of: `"initializing"`, `"idle"`, `"running"`, `"error"`, `"closed"`. `closed` can mean an unarchived, resumable task whose live provider runtime was released; terminal archive is represented separately by `archivedAt`.                                                                                                                                                       |
...
| `timeline`           | `AgentTimelineItem[]?`                   | Last durable timeline snapshot written when a live runtime closes. Archived activity reads use this snapshot without resuming the provider runtime. Legacy records may omit it; retained/durable activity is backfilled when available, otherwise archived activity is reported as unavailable rather than as a complete empty history.                                             |
