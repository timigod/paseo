...
interface SessionTestAccess {
  ...
  createAgentLifecycleDispatch: {
    createWorktreeForRequest(input: unknown): Promise<unknown>;
  ...
  workspaceProvisioning: {
    resolveOrCreateWorkspaceIdForCreateAgent(input: unknown): Promise<string>;
  };
  ...
  ): Promise<{ sessionConfig: AgentSessionConfig }>;
...
class InitialPromptEventTestClient extends CreateAgentTestClient {
  ...
  constructor(private readonly beforeCreateSession?: () => Promise<void>) {
    super();
  }
...
function createDurableCreateAgentHarness(options: {
  ...
  const session = asTestSession(
    new Session({
      ...
      tts: null,
      providerSnapshotManager,
      providerUsageService: undefined as never,
...
test("concurrent create requests acknowledge each session before forwarding the shared agent", async () => {
  const workdir = mkdtempSync(path.join(tmpdir(), "paseo-create-agent-ack-"));
  ...
  const owner = createDurableCreateAgentHarness({
    workdir,
    cwd: workdir,
    client,
    onMessage: (message) => recordCreateVisibilityOrder(ownerOrder, "req-create-owner", message),
  });
  const follower = createDurableCreateAgentHarness({
    workdir,
    cwd: workdir,
    client,
    agentManager: owner.agentManager,
    agentStorage: owner.agentStorage,
    onMessage: (message) => recordCreateVisibilityOrder(followerOrder, "req-create-owner", message),
  });

  try {
    const request = {
      type: "create_agent_request" as const,
      requestId: "req-create-owner",
...
test("create retry replays from durable storage after restart and rejects changed input", async () => {
  ...
  const initial = createDurableCreateAgentHarness({
    workdir,
    cwd: workdir,
  ...
  const request = {
    type: "create_agent_request" as const,
    requestId: "req-create-restart-replay",
    config: { provider: "codex", cwd: workdir },
    attachments: [],
  };

  try {
    await initial.session.handleMessage(request);
    const initialCreated = filterByType(initial.emitted, "status").find(
      (message) =>
        message.payload.status === "agent_created" &&
        message.payload.requestId === request.requestId,
    );
    expect(initialCreated?.payload.agentId).toEqual(expect.any(String));
    expect(initialClient.createSessionCallCount).toBe(1);

    await initial.session.cleanup();
    initial.agentManager.prepareForShutdown();
    await Promise.all(
      initial.agentManager.listAgents().map((agent) => initial.agentManager.closeAgent(agent.id)),
    );
    await initial.agentManager.flushForShutdown();
    await initial.agentStorage.flush();

    const restartedClient = new InitialPromptEventTestClient();
    const restartedManager = new AgentManager({
      clients: { codex: restartedClient },
      registry: initial.agentStorage,
      logger: asSessionLogger(createTestLogger()),
    });
    const restarted = createDurableCreateAgentHarness({
      workdir,
      cwd: workdir,
      client: restartedClient,
      agentManager: restartedManager,
      agentStorage: initial.agentStorage,
    });

    await restarted.session.handleMessage(request);
    expect(
      filterByType(restarted.emitted, "status").map((message) => message.payload),
    ).toContainEqual(
      expect.objectContaining({
        status: "agent_created",
        requestId: request.requestId,
        agentId: initialCreated?.payload.agentId,
      }),
    );
    expect(restartedClient.createSessionCallCount).toBe(0);

    await restarted.session.handleMessage({
      ...request,
      config: { ...request.config, title: "Changed retry input" },
    });
    expect(
      filterByType(restarted.emitted, "status").map((message) => message.payload),
    ).toContainEqual(
    ...
    await restarted.session.cleanup();
...
test("installed-session acknowledgement failure settles owner and follower before hanging close", async () => {
  ...
  const agentManager = new AgentManager({
    ...
    registry: agentStorage,
    logger: asSessionLogger(createTestLogger()),
    rescueTimeouts: { createAcknowledgementCloseMs: 10 },
  });
  const ownerHarness = createDurableCreateAgentHarness({
    workdir,
    cwd,
  ...
  try {
    const ownerCreating = ownerSession.handleMessage(request);
    const followerCreating = followerSession.handleMessage({ ...request });
    await expect(Promise.all([ownerCreating, followerCreating])).resolves.toEqual([
      undefined,
      undefined,
    ]);
