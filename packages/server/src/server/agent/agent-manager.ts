...
interface AgentManagerRescueTimeouts {
  ...
  historyHydrationCancelMs?: number;
}
...
interface ActiveHistoryHydration {
  ...
  providerSubagentHistoryOverlapIndex: number;
}
...
interface HistoryHydrationRequest {
  token: symbol;
  incarnation: string;
  acceptingDemand: boolean;
  acceptingBroadcastDemand: boolean;
  force: boolean;
  broadcasts: Array<boolean | (() => boolean)>;
  providerReadStarted: boolean;
  providerReadCompleted: boolean;
...
export class AgentManager {
  ...
  private readonly agentIncarnations = new Map<string, string>();
  ...
  private readonly historyHydrationRequests = new Map<string, HistoryHydrationRequest>();
  ...
  private readonly closingTimelineDrainAgentIds = new Set<string>();
  ...
  constructor(options: AgentManagerOptions) {
    ...
    this.rescueTimeouts = {
      ...
      historyHydrationCancelMs:
        options.rescueTimeouts?.historyHydrationCancelMs ?? RELOAD_SESSION_CLOSE_TIMEOUT_MS,
    };
      ...
      onFlush: ({ agentId, item, provider, turnId }) => {
        ...
        const historyHydrationToken =
          ...
          (activeHydration?.preGateCoalescerOpen
            ...
            : undefined);
        const previous = this.coalescedTimelineWriteTails.get(agentId) ?? Promise.resolve();
        const write = previous
          .catch(() => undefined)
          .then(() =>
            this.runOrBufferTimelineWriter(
              agentId,
              async () => {
        ...
        this.coalescedTimelineWriteTails.set(agentId, write);
        this.trackBackgroundTask(write);
        void write.finally(() => {
          if (this.coalescedTimelineWriteTails.get(agentId) === write) {
            this.coalescedTimelineWriteTails.delete(agentId);
          }
        });
      },
    });
  ...
  getAgentCallerIdentity(agentId: string): AgentCallerIdentity | null {
    ...
    if (!incarnation || !this.agents.has(agentId)) {
      return null;
    }
    return { agentId, incarnation };
  }
  ...
  isCurrentAgentIncarnation(agentId: string, incarnation: string): boolean {
    return (
      ...
      this.agentIncarnations.get(agentId) === incarnation
    );
  }
  ...
  reloadAgentSession(
    ...
    return this.trackWorkspaceAgentRegistration(this.agents.get(agentId)?.workspaceId, () =>
      this.trackAgentRegistrationOperation(
  ...
  private async reloadAgentSessionWithStableMembership(
    ...
    await this.cancelAndJoinHistoryHydration(agentId, "agent reloaded");
    await this.awaitDurableTimelineObligations(agentId);
    ...
    return this.withManagedWorktreeWriter(
      ...
      async () => {
        ...
        try {
          ...
          // Preserve existing labels, timeline, and provider-child projection during reload.
          // Disk refresh replaces those only after complete provider history commits.
          handedToRegistration = true;
  ...
  private async closeAgentRuntime(
    ...
    await this.cancelAndJoinHistoryHydration(agentId, "agent closed");
    this.closingTimelineDrainAgentIds.add(agentId);
    try {
      await this.drainSessionEvents(agentId);
      await this.flushCoalescedTimelineItems(agentId);
    } finally {
      this.closingTimelineDrainAgentIds.delete(agentId);
    }
    await this.awaitDurableTimelineObligations(agentId);
    await recheck?.();
    ...
    const dispatch = (event: AgentStreamEvent): void => {
      ...
      if (event.type === "timeline") {
        ...
        void this.runOrBufferTimelineWriter(
          agent.id,
          async () => {
            await this.recordTimeline(agent.id, event.item, undefined, (row) => {
          ...
          this.logger.error({ err, agentId }, "Failed to persist out-of-band timeline item");
        });
        return;
      }
  ...
  async hydrateTimelineFromProvider(
    ...
    if (existing) {
      ...
      if (forceAccepted) existing.force = true;
      const broadcastNeedsHandoff =
        options?.broadcast !== undefined && !existing.acceptingBroadcastDemand;
      const forceNeedsHandoff = options?.force === true && !forceAccepted;
      if (broadcastNeedsHandoff || forceNeedsHandoff) {
        await existing.promise;
        return await this.hydrateTimelineFromProvider(agentId, {
          ...options,
          force: forceNeedsHandoff,
        });
      }
      if (options?.broadcast !== undefined) existing.broadcasts.push(options.broadcast);
      return await existing.promise;
    }

    const hydrationToken = this.beginHistoryHydration(agentId);
    const activeHydration = this.requireLiveHistoryHydration(agentId, hydrationToken);
    const request = {
      token: hydrationToken,
      incarnation: activeHydration.incarnation,
      acceptingDemand: true,
      acceptingBroadcastDemand: true,
      force: options?.force ?? false,
      broadcasts: options?.broadcast === undefined ? [] : [options.broadcast],
      providerReadStarted: false,
      providerReadCompleted: false,
      promise: Promise.resolve(),
    } satisfies HistoryHydrationRequest;
    request.promise = this.runSerializedHistoryHydration(agentId, request).finally(() => {
      if (this.historyHydrationRequests.get(agentId) === request) {
        this.historyHydrationRequests.delete(agentId);
      }
      if (this.historyHydrationTails.get(agentId) === request.promise) {
        this.historyHydrationTails.delete(agentId);
      }
    });
    this.historyHydrationRequests.set(agentId, request);
    this.historyHydrationTails.set(agentId, request.promise);
    return await request.promise;
  }

  private async runSerializedHistoryHydration(
    agentId: string,
    request: HistoryHydrationRequest,
  ): Promise<void> {
    let released = false;
    try {
      await this.drainSessionEvents(agentId);
      this.flushPreGateCoalescedTimelineItems(agentId, request.token);
      const shouldBroadcast = () =>
        request.broadcasts.some((value) => (typeof value === "function" ? value() : value));
      ...
      while (true) {
        ...
        if (!initialPassComplete || (request.force && !request.providerReadCompleted)) {
          ...
          request.providerReadStarted = force || !agent.historyPrimed;
          if (request.providerReadStarted) activeHydration.historyReadOpen = true;
          performedThisPass = await this.hydrateTimelineFromLegacyProviderHistory(
            agent,
            { force, broadcast },
            activeHydration,
          );
          request.providerReadCompleted ||= performedThisPass;
          broadcastedThisPass = performedThisPass && broadcast;
          initialPassComplete = true;
        }
        if (shouldBroadcast() && !broadcastedThisPass) {
          this.broadcastCurrentHistorySnapshot(agent);
        }
        request.acceptingBroadcastDemand = false;
        if (!request.providerReadCompleted) activeHydration.historyReadOpen = true;
        released = await this.releaseHistoryHydration(agentId, request.token, () => {
          const demandSatisfied = !request.force || request.providerReadCompleted;
          if (demandSatisfied) request.acceptingDemand = false;
          return demandSatisfied;
        });
        if (released) return;
        activeHydration.historyReadOpen = true;
      }
    } finally {
      request.acceptingDemand = false;
      request.acceptingBroadcastDemand = false;
      const active = this.activeHistoryHydrations.get(agentId);
      if (active?.token === request.token && !active.canceled) {
        await this.releaseHistoryHydration(agentId, request.token);
      }
    }
  }
  ...
  async deleteAgentState(agentId: string): Promise<void> {
    await this.cancelAndJoinHistoryHydration(agentId, "agent state deleted");
    await this.awaitDurableTimelineObligations(agentId);
    this.discardRetainedAgentState(agentId);
  ...
  private async getLastAssistantMessageFromStores(agentId: string): Promise<string | null> {
    ...
    if (liveMessage !== null || !this.durableTimelineStore) return liveMessage;
  ...
  private async registerSession(
    ...
    try {
      ...
      const now = new Date();
      // Timeline initialization mutates the in-memory store before its awaited durable seed.
      // Own rollback before entering that fallible boundary.
  ...
  private async initializeAgentTimelineForRegister(params: {
    ...
    if (options?.timelineRows?.length) {
      await this.insertInitialDurableTimelineRows(agentId, options.timelineRows);
    }
  ...
  private prepareAgentForClosure(
    ...
    this.agentIncarnations.delete(agent.id);
  ...
  private discardRetainedAgentState(agentId: string): void {
    this.cancelHistoryHydration(agentId, "retained state discarded");
  ...
  private subscribeToSession(agent: ActiveManagedAgent): void {
    ...
    const incarnation = this.agentIncarnations.get(agentId);
    if (!incarnation) {
      throw new Error(`Agent ${agentId} has no live incarnation for event subscription`);
    }
    const unsubscribe = agent.session.subscribe((event: AgentStreamEvent) => {
      this.enqueueSessionEvent(agentId, incarnation, event);
    });
    agent.unsubscribeSession = unsubscribe;
  }

  private enqueueSessionEvent(agentId: string, incarnation: string, event: AgentStreamEvent): void {
    if (!this.isCurrentAgentIncarnation(agentId, incarnation)) return;
    if (this.closingAgentIds.has(agentId)) return;
    const activeHydration = this.activeHistoryHydrations.get(agentId);
    if (activeHydration?.incarnation === incarnation && !activeHydration.canceled) {
      activeHydration.bufferedOperations.push({
        kind: "session_event",
        run: async () =>
          this.processSessionEvent(agentId, incarnation, event, activeHydration.token),
        ...(event.type === "timeline"
          ? {
              onOverlap: () =>
                this.applyOverlappedTimelineOperationalEffects(agentId, incarnation, event),
            }
          : {}),
        ...
        ...(event.type === "provider_subagent"
          ...
          : {}),
        eligibleForHistoryOverlap: activeHydration.historyReadOpen,
      });
      return;
    }
    this.logger.trace(
    ...
    const next = previous
      .catch(() => undefined)
      .then(async () => this.processSessionEvent(agentId, incarnation, event, admissionToken))
      .catch((err) => {
        this.logger.error(
    ...
    void next.finally(() => {
      ...
      if (tokens?.size === 0) this.sessionEventAdmissionTokens.delete(agentId);
  ...
  private async applyOverlappedTimelineOperationalEffects(
    agentId: string,
    incarnation: string,
    event: Extract<AgentStreamEvent, { type: "timeline" }>,
  ): Promise<void> {
    if (!this.isCurrentAgentIncarnation(agentId, incarnation)) return;
    await this.applyTimelineOperationalEffects(agentId, event, true);
  }

  private async applyTimelineOperationalEffects(
    agentId: string,
    event: Extract<AgentStreamEvent, { type: "timeline" }>,
    notifyWaiters: boolean,
  ): Promise<void> {
    const agent = this.agents.get(agentId);
    if (!agent || agent.session == null) return;
    ...
    if (
      ...
      commandMayHaveChangedExternalState(event.item.detail.command)
    ) {
      this.onWorkspaceStateMayHaveChanged?.({ cwd: agent.cwd });
    }
    if (notifyWaiters) this.runs.notifyAgentWaiters(agent, event);
  }
  ...
  private async hydrateTimelineFromLegacyProviderHistory(
    ...
    if (agent.historyPrimed && !options?.force) {
      activeHydration.historyReadOpen = false;
      return false;
    }

    const broadcast = options?.broadcast ?? false;

    // An unprimed agent with existing rows is a staged refresh or retry. It replaces provider
    // history atomically instead of appending to the last complete incarnation.
    if (options?.force || this.timelineStore.getRows(agent.id).length > 0) {
      await this.forceHydrateTimelineFromLegacyProviderHistory(agent, broadcast, activeHydration);
      return true;
    }
  ...
  private replaceProviderSubagentHistory(
    ...
    for (const event of this.providerSubagents.deleteParent(agentId)) {
      if (broadcast) this.dispatch({ type: "provider_subagent", event });
    }
    for (const event of providerSubagentEvents) {
      const update = this.providerSubagents.apply(agentId, event.provider, event.event);
      if (broadcast) this.dispatch({ type: "provider_subagent", event: update });
    }
  ...
  private broadcastHydratedTimeline(
    ...
    for (let index = 0; index < historyEvents.length; index += 1) {
      ...
      this.dispatchStream(agent.id, event, { seq: row.seq, epoch, timestamp: row.timestamp });
    }
  ...
  private broadcastCurrentHistorySnapshot(agent: ActiveManagedAgent): void {
  ...
  private async primeTimelineFromLegacyProviderHistory(
    ...
    activeHydration.nextUncapturedTimelineSeq = (replacementRows.at(-1)?.seq ?? 0) + 1;
    activeHydration.lastProviderHistoryItems = historyRows.map((row) => structuredClone(row.item));
    activeHydration.lastProviderSubagentHistory = providerSubagentEvents.map((event) => ({
      provider: event.provider,
      event: structuredClone(event.event),
    }));
    agent.materialProgress = materialProgress;
    ...
    for (const event of providerSubagentEvents) {
      const update = this.providerSubagents.apply(agent.id, event.provider, event.event);
      if (shouldBroadcast) this.dispatch({ type: "provider_subagent", event: update });
    }
  ...
  private mergeCarriedProviderSubagentEvents(
    activeHydration: ActiveHistoryHydration,
    incomingHistory: Array<{ provider: AgentProvider; event: ProviderSubagentInputEvent }>,
    ...
    const unmatched: Array<{ provider: AgentProvider; event: ProviderSubagentInputEvent }> = [];
    for (const carriedEvent of activeHydration.carriedProviderSubagentEvents) {
      ...
      if (matchingHistoryIndex < 0) unmatched.push(structuredClone(carriedEvent));
      else historySearchIndex = matchingHistoryIndex + 1;
    }
    return unmatched;
  }
  ...
  private beginHistoryHydration(agentId: string): symbol {
    if (this.activeHistoryHydrations.has(agentId)) {
      throw new Error(`Agent ${agentId} already has an admitted history hydration`);
    }
    const incarnation = this.agentIncarnations.get(agentId);
    if (!incarnation || !this.agents.has(agentId)) {
      throw new Error(`Agent ${agentId} has no live incarnation for history hydration`);
    }
  ...
  private requireLiveHistoryHydration(agentId: string, token: symbol): ActiveHistoryHydration {
    const active = this.activeHistoryHydrations.get(agentId);
    if (
      !active ||
      active.token !== token ||
      active.canceled ||
      !this.isCurrentAgentIncarnation(agentId, active.incarnation)
    ) {
  ...
  private cancelHistoryHydration(agentId: string, reason: string): Promise<void> | null {
    const active = this.activeHistoryHydrations.get(agentId);
    if (active) {
      active.canceled = true;
      this.activeHistoryHydrations.delete(agentId);
      this.agentStreamCoalescer.flushAndDiscard(agentId);
      const error = new Error(`Agent ${agentId} history hydration canceled: ${reason}`);
      for (const operation of [...active.preGateBufferedOperations, ...active.bufferedOperations]) {
        operation.reject?.(error);
      }
      active.preGateBufferedOperations.length = 0;
      active.bufferedOperations.length = 0;
    }
    return this.historyHydrationRequests.get(agentId)?.promise ?? null;
  }

  private async cancelAndJoinHistoryHydration(agentId: string, reason: string): Promise<void> {
    const pending = this.cancelHistoryHydration(agentId, reason);
    if (!pending) return;
    const result = await this.waitWithTimeout({
      operation: pending.catch(() => undefined),
      timeoutMs: this.rescueTimeouts.historyHydrationCancelMs,
    });
    if (result === "completed") return;
    const request = this.historyHydrationRequests.get(agentId);
    if (request?.promise === pending) this.historyHydrationRequests.delete(agentId);
    if (this.historyHydrationTails.get(agentId) === pending) {
      this.historyHydrationTails.delete(agentId);
    }
    this.logger.warn(
      { agentId, reason, timeoutMs: this.rescueTimeouts.historyHydrationCancelMs },
      "Timed out joining canceled provider history hydration",
    );
  }
  ...
  private async releaseHistoryHydration(
    ...
    const activeHydration = this.requireLiveHistoryHydration(agentId, token);

    await this.drainSessionEvents(agentId);
    while (true) {
      this.requireLiveHistoryHydration(agentId, token);
      const operation =
        activeHydration.preGateBufferedOperations.shift() ??
        activeHydration.bufferedOperations.shift();
      if (operation) {
        await this.replayBufferedHistoryHydrationOperation(
          agentId,
          token,
          activeHydration,
          operation,
      ...
      await this.flushCoalescedTimelineItems(agentId, token);
      if (this.hasBufferedHistoryHydrationOperations(activeHydration)) {
        continue;
      }
      ...
      await this.finalizeHistoryPrimedAfterHydration(agentId);
      if (this.hasBufferedHistoryHydrationOperations(activeHydration)) {
        continue;
      }
      this.requireLiveHistoryHydration(agentId, token);
  ...
  private hasBufferedHistoryHydrationOperations(active: ActiveHistoryHydration): boolean {
    return active.preGateBufferedOperations.length > 0 || active.bufferedOperations.length > 0;
  }

  private async replayBufferedHistoryHydrationOperation(
    agentId: string,
    token: symbol,
    activeHydration: ActiveHistoryHydration,
    ...
    try {
      if (this.consumeHistoryHydrationOverlap(activeHydration, operation)) {
        await operation.onOverlap?.();
        operation.resolve?.();
        return;
      }
      if (operation.kind === "timeline_writer") {
        this.withCoalescerHistoryHydrationToken(agentId, token, () => {
  ...
  private async finalizeHistoryPrimedAfterHydration(agentId: string): Promise<void> {
    const agent = this.agents.get(agentId);
    if (
      !agent ||
      !("session" in agent) ||
  ...
  private consumeHistoryHydrationOverlap(
    activeHydration: ActiveHistoryHydration,
    operation: BufferedHistoryHydrationOperation,
  ): boolean {
    ...
    if (operation.providerSubagentEvent) {
      const match = activeHydration.lastProviderSubagentHistory.findIndex(
        (event, index) =>
          index >= activeHydration.providerSubagentHistoryOverlapIndex &&
          isDeepStrictEqual(event, operation.providerSubagentEvent),
      );
      if (match >= 0) {
        activeHydration.providerSubagentHistoryOverlapIndex = match + 1;
        return true;
      }
    }
    return false;
  ...
  private runOrBufferHistoryHydrationOperation(
    ...
    if (
      this.closingAgentIds.has(agentId) &&
      historyHydrationToken === undefined &&
      !this.closingTimelineDrainAgentIds.has(agentId)
    ) {
      return Promise.reject(new Error(`Agent ${agentId} is closing`));
    }
    const activeHydration = this.activeHistoryHydrations.get(agentId);
    ...
    if (
      ...
      activeHydration.preGateBufferedOperations.push({
        ...operation,
        eligibleForHistoryOverlap:
          operation.eligibleForHistoryOverlap ?? activeHydration.historyReadOpen,
      });
  ...
  private async flushCoalescedTimelineItems(
    agentId: string,
    historyHydrationToken?: symbol,
  ): Promise<void> {
    ...
    while (true) {
      const tail = this.coalescedTimelineWriteTails.get(agentId);
      if (!tail) return;
      await tail;
      if (this.coalescedTimelineWriteTails.get(agentId) === tail) return;
    }
  }
  ...
  private withCoalescerHistoryHydrationToken<T>(
    ...
    if (!historyHydrationToken) return run();
    ...
    try {
      ...
      if (previousToken) this.coalescerHistoryHydrationTokens.set(agentId, previousToken);
      else this.coalescerHistoryHydrationTokens.delete(agentId);
    }
  }
  ...
  private async handleStreamEvent(
    ...
    if (!options?.fromHistory) {
      ...
      await this.flushCoalescedTimelineItems(agent.id, options?.historyHydrationToken);
    }
  ...
  private async recordAndDispatchTimelineItem(
    ...
    await this.recordTimeline(agentId, item, turnId ? { turnId } : undefined, async (row) => {
      ...
      await this.applyTimelineOperationalEffects(agentId, event, false);
    });
  ...
  private recordTimeline(
    ...
    if (!this.durableTimelineStore) {
      return this.recordTimelineWithDurability(agentId, item, options, afterCommit);
    }
    const previous = this.durableTimelinePrimingTasks.get(agentId) ?? Promise.resolve();
    const operation = previous
      .catch(() => undefined)
      .then(() => this.recordTimelineWithDurability(agentId, item, options, afterCommit));
    const settled = operation.then(
      () => undefined,
      () => undefined,
    );
    this.durableTimelinePrimingTasks.set(agentId, settled);
    void settled.finally(() => {
      if (this.durableTimelinePrimingTasks.get(agentId) === settled) {
        this.durableTimelinePrimingTasks.delete(agentId);
      }
    });
    return operation;
  ...
  private async recordTimelineWithDurability(
    ...
    const inheritedIncomplete = this.incompleteDurableTimelineHistories.has(agentId);
    this.incompleteDurableTimelineHistories.add(agentId);
    if (agent && this.agents.get(agentId) === agent) {
      agent.historyPrimed = false;
      await this.persistSnapshot(agent);
    }
    const version = (this.durableTimelineWriteVersions.get(agentId) ?? 0) + 1;
  ...
  private async commitDurableTimelineAppend(
    ...
    return await this.queueDurableTimelineWrite(agentId, async () =>
      store.appendCommitted(agentId, item, options),
    );
  }

  private async insertInitialDurableTimelineRows(
    agentId: string,
  ...
  private async awaitDurableTimelineObligations(agentId: string): Promise<void> {
    while (true) {
      const write = this.durableTimelineWriteTails.get(agentId);
      const priming = this.durableTimelinePrimingTasks.get(agentId);
      if (!write && !priming) return;
      await Promise.allSettled([...(write ? [write] : []), ...(priming ? [priming] : [])]);
      if (
        this.durableTimelineWriteTails.get(agentId) === write &&
        this.durableTimelinePrimingTasks.get(agentId) === priming
      ) {
        return;
      }
    }
  ...
  private trackAgentRegistrationResult<T>(result: Promise<T>): Promise<T> {
  ...
  runWorkspaceAgentRegistration<T>(
    ...
    return this.trackAgentRegistrationResult(registration);
  }
  ...
  private async flushTasks(options: { includeAgentRegistrations: boolean }): Promise<void> {
    ...
    // Drain tasks and serialized provider/durable obligations, including successors spawned while
    // awaiting. A flush receipt must never race a hydration or timeline write admitted earlier.
    while (true) {
      const pending = options.includeAgentRegistrations
        ? [
            ...this.backgroundTasks,
            ...this.agentRegistrationTasks,
            ...this.historyHydrationTails.values(),
            ...this.durableTimelineWriteTails.values(),
            ...this.durableTimelinePrimingTasks.values(),
            ...this.coalescedTimelineWriteTails.values(),
          ]
        : [
            ...this.backgroundTasks,
            ...this.historyHydrationTails.values(),
  ...
  private async buildLaunchContext(
    ...
    if (
      ...
      context.paseoTools = await this.paseoToolCatalogFactory({
        ...
        callerAgentIncarnation: agentIncarnation,
      });
    }
    return context;
  }
