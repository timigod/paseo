...
export interface AgentTimelineStore {
  ...
  getHistoryQuarantineStartSeq(agentId: string): Promise<number | null>;
  appendHistoryQuarantined(agentId: string, row: AgentTimelineRow, startSeq: number): Promise<void>;
  clearHistoryQuarantine(agentId: string): Promise<void>;
